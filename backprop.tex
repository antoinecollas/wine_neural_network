...
avec 
    ...
    \begin{equation}
        \frac{\partial log(\hat{p}_{k}^{(p)})}{\partial (x^{(p,2)})_j} = \sum\limits_{k=1}^{n_{out}} \frac{\partial log(\hat{p}_{k}^{(p)})}{\partial (x^{(p,3)})_k} \frac{\partial (x^{(p,3)})_k}{\partial (x^{(p,2)})_j}
        \label{log_x2}
    \end{equation}
    ...
    \begin{equation}
        \frac{\partial log(\hat{p}_{k}^{(p)})}{\partial (x^{(p,2)})_j} = \frac{1}{ln(10)} \sum\limits_{k=1}^{n_{out}} \bigg[ \delta_{kj} - softmax(x^{(p,3)})_j \bigg] w_{jk}^{(2)}
        \label{log_x2_final}
    \end{equation}
    ...
    \begin{equation}
        \frac{\partial log(\hat{p}_{k}^{(p)})}{\partial w_{ij}^{(1)}} = \frac{1}{ln(10)} \sum\limits_{k=1}^{n_{out}} \bigg[ \delta_{kj} - softmax(x^{(p,3)})_j \bigg] w_{jk}^{(2)} H\bigg[\sum\limits_{k=1}^{n_{in}} w_{kj}^{(1)}(x^{(p,1)})_{k} + b_j^{(1)}\bigg](x^{(p,1)})_i
        \label{log_wij_final}
    \end{equation}

    D'après les équations \eqref{E_wij1} et \eqref{log_wij_final}:
    \begin{equation}
        \frac{\partial E}{\partial w_{ij}^{(1)}} = -\frac{1}{m \times ln(10)}  \sum\limits_{k=1}^{n_{out}} \sum\limits_{p=1}^{m} \Bigg[ y_k^{(p)} \sum\limits_{k=1}^{n_{out}} \bigg[ \big[ \delta_{kj} - softmax(x^{(p,3)})_j \big] w_{jk}^{(2)} \bigg] H\big[\sum\limits_{k=1}^{n_{in}} w_{kj}^{(1)}(x^{(p,1)})_{k} + b_j^{(1)}\big](x^{(p,1)})_i \Bigg]
    \end{equation}
    ...
    \begin{itemize}
        ...
        \item la modification est aussi proportionnelle aux erreurs de toutes les sorties (les couches sont totalement connectées ce qui est visible dans $\sum\limits_{k=1}^{n_{out}} \bigg[ \big[ \delta_{kj} - softmax(x^{(p,3)})_j \big] w_{jk}^{(2)} \bigg]$)
    \end{itemize}
    % \begin{equation}
